...
va_list vap)
{
    extern HSim__s6 * createworkM_m_u_x4_t_bifelse__v(const char*);
    HSim__s6 *blk = createworkM_m_u_x4_t_bifelse__v(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","MUX4TBifelse_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","mux4", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_m_u_x4_t_bifelse__v(const char*);
        topvl = (HSim__s5*)createworkM_m_u_x4_t_bifelse__v("MUX4TBifelse_v");
