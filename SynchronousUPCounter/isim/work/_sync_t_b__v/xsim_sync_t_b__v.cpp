...
va_list vap)
{
    extern HSim__s6 * createworkM_sync_t_b__v(const char*);
    HSim__s6 *blk = createworkM_sync_t_b__v(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkM_sunc_u_p_m(const char*);
    HSim__s6 *blk = createworkM_sunc_u_p_m(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","SyncTB_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","dff", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","SuncUPM", (HSimInstFactoryPtr)IF2);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF3);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_sync_t_b__v(const char*);
        topvl = (HSim__s5*)createworkM_sync_t_b__v("SyncTB_v");
